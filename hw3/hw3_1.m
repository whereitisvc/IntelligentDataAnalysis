...
for i = 1:5
    test = (indices == i); 
    train = ~test;
    QDA = ClassificationDiscriminant.fit(meas(train,1:2),species(train,:),'DiscrimType','quadratic');
    predit = predict(QDA,meas(test,1:2));
...
for i = 1:5
    test = (indices == i); 
    train = ~test;
    LDA = ClassificationDiscriminant.fit(meas(train,1:2),species(train,:),'DiscrimType','linear');
    predit = predict(LDA,meas(test,1:2));
    conf_mat = confusionmat(species(test,:),predit);
    Conf_MAT = Conf_MAT + conf_mat;
...
for i = 1:5
    test = (indices == i); 
    train = ~test;
    NB = ClassificationDiscriminant.fit(meas(train,1:2),species(train,:),'DiscrimType','diagLinear');
    predit = predict(NB,meas(test,1:2));
    conf_mat = confusionmat(species(test,:),predit);
    Conf_MAT = Conf_MAT + conf_mat;
